...
from utils.utils import get_curl
...
class InvalidLogLevel(Exception):
    ...
    def __str__(self):
        return (
            f'Указан недопустимый уровень логирования: "{self.log_level}". '
...
def log_request(
    request: PreparedRequest, is_compressed: bool = False, is_insecure: bool = False
):
    """Залогировать запрос
    ...
    curl = None
    msg = (
        f"HTTP-Method: <blue><normal>{request.method}</normal></blue>\n"
    ...
    if "boundary" not in request.headers:
        curl = get_curl(
            request=request, is_compressed=is_compressed, is_insecure=is_insecure
        )
    ...
    try:
        ...
        logger.opt(colors=False).info(
            msg.replace("<blue><normal>", "").replace("</normal></blue>", "")
        )
...
def log_response(response: Response):
    ...
    Args:
        response: ответ
    """
    color = {1: "light-blue", 2: "green", 3: "yellow", 4: "red", 5: "red"}.get(
        response.status_code // 100, "y"
    )
