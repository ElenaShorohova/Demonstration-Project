...
class CustomResponse:
    ...
    def __init__(
        ...
        Args:
            ...
            response_error_model: модель ответа для негативных сценариев,
            передается в методе запроса.
        """
    ...
    def json(self, **kwargs) -> dict[str, Any] | list[dict[str, Any]] | None:
        ...
        if self._response_body is None:
            logger.debug(
                "Тело не записано внутри экземпляра. Парс тела из ответа Response"
            )

            self._response_body = self.response.json(**kwargs)

            logger.debug(
                "Результат: {}. Тело записано в экземпляр", self._response_body
            )

        else:
            logger.debug(
                "Тело уже записано внутри экземпляра. Возвращаем тело из экземпляра"
            )
    ...
    def success(self) -> bool | None:
        ...
        if body := self.json():
            if isinstance(body, dict):
                ...
                logger.debug(
                    "Получаем значение из ключа success. Полученное значение {}", result
                )

            else:
                logger.warning(
                    "Тело ответа не является словарем. Невозможно получить success!"
                    " Тип тела ответа: {}",
                    type(body),
                )
        else:
    ...
    def dto(self, is_error_model: bool = False) -> BaseModel:
        ...
        if not model_:
            raise AttributeError(
                f'{"" if is_error_model else "Негативная "}Модель не задана!'
            )
    ...
    def __base_check_expected_status_code(
        self, expected_code: int, exception: type[Exception] | None = None
    ):
    ...
    def check_status_code(
        self, expected_code: int = 200, exception: type[Exception] = ValueError
    ) -> Self:
        ...
        """
        self.__base_check_expected_status_code(
            expected_code=expected_code, exception=exception
        )

        return self

    def assert_model_valid(
        self,
        ...
        inst_model = (
            self.response_error_model if is_error_model else self.response_model
        )
        ...
        if model_ is None:
            raise AttributeError(
                f'Отсутствует {"негативная " if is_error_model else ""} '
                f"модель для валидации!"
            )
